...
class Logger(object):
    ...
    def record_tabular(self, key, val, step=0):
        self.writer.add_scalar(key, val, global_step=step)
        # self._tabular.append((self._tabular_prefix_str + str(key), str(val)))

    def record_dict(self, d, prefix=None, step=0):
        if prefix is not None:
            for k, v in d.items():
                self.writer.add_scalar(prefix + k, v, global_step=step)
        else:
            for k, v in d.items():
                self.writer.add_scalar(k, v, global_step=step)

    def log_batch(self, itr, batch, discount, prefix='Evaluation'):
        ...
        self.writer.add_scaler(f'{prefix}/Iteration', itr)
        self.writer.add_scaler(f'{prefix}/NumEpisodes', itr)
        self.writer.add_scaler(f'{prefix}/AverageDiscountedReturn', itr)
        self.writer.add_scaler(f'{prefix}/AverageReturn', itr)
        self.writer.add_scaler(f'{prefix}/StdReturn', itr)
        self.writer.add_scaler(f'{prefix}/MaxReturn', itr)
        self.writer.add_scaler(f'{prefix}/MinReturn', itr)
        self.writer.add_scaler(f'{prefix}/TerminationRate', itr)
        if success:
            self.writer.add_scaler(f'{prefix}/SuccessRate', itr)
