...
class FederatedTransformer(PyTorchModule):
    def __init__(self, hidden_sizes, output_size, input_size, transformer_layer_config, num_layers, num_agents=5, init_w=3e-3,
        ...
        super().__init__()
        self.encoders = nn.ModuleList([Mlp(hidden_sizes,
                                           hidden_sizes[-1],
                                           input_size,
                                           init_w,
                                           hidden_activation,
        ...
        self.decoders = nn.ModuleList([Mlp(hidden_sizes,
                                           output_size,
                                           hidden_sizes[-1],
                                           init_w,
                                           hidden_activation,
    ...
    def forward(self, x):
        ...
        transformed = self.transformer(encodings, encodings, encodings)
        decoded = [net(encoded) for net, encoded in
                   zip(self.decoders, transformed)]  # we use other networks to weight entropy perhaps

        return decoded[0], decoded  # return decoded for the current agent

    def fuse(self, new_encoder, new_decoder, index):
        # math here to fuse, for now, just replace
        self.encoders[index] = new_encoder
        self.decoders[index] = new_decoder
